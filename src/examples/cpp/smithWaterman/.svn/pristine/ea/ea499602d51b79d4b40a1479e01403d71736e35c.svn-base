...
int main ( int argc, char* argv[] ) {
    ...
    for ( i = 1; i < n_char_in_file_2+1; ++i ) {
		result_array[current][0] = GAP_PENALTY*i;
    	for ( j = 1; j < n_char_in_file_1+1; ++j ) {
        	signed char char_from_1 = string_1[j-1];
            signed char char_from_2 = string_2[i-1];

            int diag_score = result_array[prev   ][j-1] + alignment_score_matrix[char_from_2][char_from_1];
            int left_score = result_array[current][j-1] + alignment_score_matrix[char_from_1][GAP];
            int  top_score = result_array[prev   ][j  ] + alignment_score_matrix[GAP][char_from_2];

            int bigger_of_left_top = (left_score > top_score) ? left_score : top_score;
           	result_array[current][j] = (bigger_of_left_top > diag_score) ? bigger_of_left_top : diag_score;
		}
		int temp = prev;
		prev = current;
		current = temp;
