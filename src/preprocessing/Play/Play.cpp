...
int Play::minDistanceToNextFunctionCall(BasicBlock *curr,
        map<BasicBlock *, int> *cache, vector<BasicBlock *> *stack) {

    /*
     * If we've traversed back over a basic block without finding a function
     * call the first time, skip it.
     */
    if (std::find(stack->begin(), stack->end(), curr) != stack->end()) {
        return -1;
    }
    ...
    int count = 0;
    for (BasicBlock::iterator i = curr->begin(), e = curr->end();
            i != e; ++i) {
        Instruction *inst = &*i;

        if (CallInst *call = dyn_cast<CallInst>(inst)) {
            if (!isa<IntrinsicInst>(call) &&
                    !isKnownFunction(call->getCalledFunction())) {
                return count + 1;
            }
        }
        count++;
    ...
    int min_child_count = -1;
    TerminatorInst *term = curr->getTerminator();
    for (unsigned int i = 0; i < term->getNumSuccessors(); i++) {
        BasicBlock *child = term->getSuccessor(i);
        int child_count = minDistanceToNextFunctionCall(child, cache, stack);
        if (child_count != -1 && (min_child_count == -1 ||
                    child_count < min_child_count)) {
    ...
    if (min_child_count != -1) {
        cache->insert(pair<BasicBlock *, int>(curr, count + min_child_count));
        return count + min_child_count;
    } else {
        cache->insert(pair<BasicBlock *, int>(curr, -1));
        return -1; // no call function found
    }
}

int Play::getMinDistanceFromCall(BasicBlock::iterator call_pointer,
        BasicBlock *start) {
    call_pointer++; // increment past the call
    int count = 0;

    while (call_pointer != start->end()) {
        Instruction *inst = &*call_pointer;
    ...
    int min_child_count = -1;
    TerminatorInst *term = start->getTerminator();
    map<BasicBlock *, int> cache;
    for (unsigned int i = 0; i < term->getNumSuccessors(); i++) {
        BasicBlock *child = term->getSuccessor(i);
...
void Play::dumpFunctionCallLatency(Module &M, const char *output_file) {
    FILE *fp = fopen(output_file, "w");
        ...
        for (Function::BasicBlockListType::iterator bb_iter = bblist.begin(),
            ...
            for (BasicBlock::iterator i = bb->begin(), e = bb->end();
                ...
                if (CallInst *call = dyn_cast<CallInst>(inst)) {
                    if (!isa<IntrinsicInst>(call) &&
                            !isKnownFunction(call->getCalledFunction())) {
                        int min_dist = getMinDistanceFromCall(i, bb);
                        if (min_dist != -1 && (min_dist_for_func == -1 ||
                                    min_dist < min_dist_for_func)) {
                            min_dist_for_func = min_dist;
                        }
                    }
                }
            }
...
bool Play::runOnModule(Module &M) {
    ...
    dumpFunctionCallLatency(M, "latency.info");
