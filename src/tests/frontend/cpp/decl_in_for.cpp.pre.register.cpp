...
int main(int argc, char **argv) {
    init_numdebug(0); new_stack();  register_stack_var("main|argc|0", "i32", &argc, 4, 0, 0, 0);  register_stack_var("main|argv|0", "i8**", &argv, 8, 1, 0, 0);  int a; lbl_0:  register_stack_var("main|a|0", "i32", &a, 4, 0, 0, 0);  if (____numdebug_replaying) { goto lbl_1; }   a = 0; 
    {  int i; lbl_1:  register_stack_var("main|i|0", "i32", &i, 4, 0, 0, 0);  if (____numdebug_replaying) { goto lbl_2; } lbl_2: if (____numdebug_replaying) { int dst = get_next_call(); switch(dst) { case(0): { goto call_lbl_0; } default: { fprintf(__stderrp, "Unknown label %d at %s:%d\n", dst, __FILE__, __LINE__); exit(1); } } }  for ( i = 0;  i < 10; alias_group_changed(1, 4), i++) {
        a += i;
    } }
     call_lbl_0: calling(0); checkpoint();
    rm_stack(); return 0;
}
